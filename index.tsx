...
// --- Configuration ---
// Simple obfuscation for storage as requested (Encryption)
...
const getCustomKey = () => {
    const stored = localStorage.getItem('suno_custom_api_key');
    return stored ? decryptKey(stored) : null;
};

// Prioritize Custom Key -> Environment Key
const getGenAI = () => new GoogleGenAI({ apiKey: getCustomKey() || process.env.API_KEY });
...
const ApiKeySettingsModal = ({ onClose }: { onClose: () => void }) => {
    ...
    const handleTestAndSave = async () => {
        ...
        try {
            ...
            // If successful
            localStorage.setItem('suno_custom_api_key', encryptKey(apiKey.trim()));
            setStatus('SUCCESS');
            setMsg('연결 성공! 키가 안전하게 저장되었습니다.');
            ...
            console.error(e);
            setStatus('FAILURE');
            setMsg(`연결 실패: ${e.message || 'API Key를 확인해주세요.'}`);
        }
    };

    const handleClear = () => {
        localStorage.removeItem('suno_custom_api_key');
        setApiKey('');
        setMsg('저장된 키가 삭제되었습니다.');
        setStatus('IDLE');
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                width: '500px', maxWidth: '90vw', border: '1px solid #374151',
                boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)'
            }} onClick={e => e.stopPropagation()}>
                <h2 style={{ marginTop: 0, color: 'white', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span className="material-symbols-outlined" style={{ color: '#fbbf24' }}>vpn_key</span>
                    API Key 관리 (External)
                </h2>
                <p style={{ color: '#9ca3af', fontSize: '14px', marginBottom: '20px' }}>
                    Google AI Studio API Key를 입력하세요. 키는 브라우저 로컬 스토리지에 암호화되어 저장됩니다.
                </p>
                ...
                <div style={{ marginBottom: '20px' }}>
                    <input 
                        ...
                        style={{ 
                            ...
                            border: '1px solid #4b5563', backgroundColor: '#111827', 
                ...
                {msg && (
                    <div style={{ 
                        marginBottom: '20px', padding: '10px', borderRadius: '6px', fontSize: '13px',
                        backgroundColor: status === 'SUCCESS' ? 'rgba(16, 185, 129, 0.2)' : (status === 'FAILURE' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(59, 130, 246, 0.2)'),
                        color: status === 'SUCCESS' ? '#34d399' : (status === 'FAILURE' ? '#f87171' : '#60a5fa'),
                        border: `1px solid ${status === 'SUCCESS' ? '#059669' : (status === 'FAILURE' ? '#dc2626' : '#2563eb')}`
                    }}>
                        {msg}
                    </div>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <button 
                        onClick={handleClear}
                        style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', textDecoration: 'underline' }}
                    >
                        저장된 키 삭제
                    </button>
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <button 
                            onClick={onClose}
                            style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid #4b5563', background: 'transparent', color: '#d1d5db', cursor: 'pointer' }}
                        >
                            취소
                        </button>
                        <button 
                            ...
                            style={{ 
                                ...
                                backgroundColor: '#e11d48', color: 'white', fontWeight: 'bold', cursor: 'pointer',
                                display: 'flex', alignItems: 'center', gap: '8px'
                            }}
                        >
                            {status === 'TESTING' ? '테스트 중...' : '연결 테스트 & 저장'}
                        </button>
                    </div>
                </div>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
          <div>
            <div style={{ marginBottom: '25px' }}>
                ...
                style={{ width: '100%', height: '120px', padding: '15px', borderRadius: '8px', backgroundColor: '#111827', border: '1px solid #374151', color: 'white', resize: 'none' }}
                />
            </div>
            ...
            <div style={{ marginBottom: '25px' }}>
                ...
                style={{ width: '100%', height: '120px', padding: '15px', borderRadius: '8px', backgroundColor: '#111827', border: '1px solid #e11d48', color: 'white', resize: 'none' }}
                />
            </div>
          </div>
          ...
          <div>
             <div style={{ marginBottom: '25px' }}>
                ...
                <input 
                    ...
                    style={{ width: '100%', padding: '15px', borderRadius: '8px', backgroundColor: '#111827', border: '1px solid #374151', color: 'white', fontSize: '18px', fontWeight: 'bold' }}
                />
             ...
             <div style={{ padding: '20px', backgroundColor: '#1f2937', borderRadius: '12px', border: '1px solid #374151' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <input 
                        ...
                        style={{ width: '100%', padding: '12px', backgroundColor: '#111827', border: '1px solid #4b5563', color: 'white', borderRadius: '8px' }}
                    />
                    <input 
                        type="text" 
...
const StructureTab = ({ project, onUpdate, legibilityMode }: any) => {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', overflowX: 'auto', paddingBottom: '20px', alignItems: 'flex-start' }}>
            {project.structure.map((block: SongBlock, i: number) => (
                <div key={block.id} style={{ 
                    ...
                    <input 
                        ...
                        onChange={(e) => updateBlockDescription(i, e.target.value)}
                        placeholder="직접 입력..."
                        style={{ width: '100%', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', fontSize: '12px', padding: '4px', borderRadius: '4px' }}
                    />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ marginTop: '10px', padding: '10px', backgroundColor: '#1f2937', borderRadius: '6px' }}>
                        ...
                        <input 
                            ...
                            style={{ width: '100%', padding: '8px', backgroundColor: '#374151', border: 'none', color: 'white', borderRadius: '4px', fontSize: '13px' }}
                        />
...
const LyricsTab = ({ project, onUpdate, legibilityMode }: any) => {
  ...
  return (
    <div style={{ width: '100%', height: 'calc(100vh - 150px)', display: 'grid', gridTemplateColumns: '320px 360px 1fr', gap: '20px', minHeight: '600px' }}>
            ...
            >
             ...
             <div>
                ...
                <input 
                    ...
                    style={{ width: '100%', padding: '10px', backgroundColor: '#374151', border: 'none', borderRadius: '6px', color: 'white', fontSize: '13px' }}
                />
            </div>
...
const SoundTab = ({ project, onUpdate, legibilityMode }: any) => {
  ...
  return (
      <div style={{ width: '100%', height: 'calc(100vh - 150px)', display: 'grid', gridTemplateColumns: '3fr 2fr 5fr', gap: '20px', minHeight: '600px' }}>
                  ...
                  <div style={{ marginBottom: '15px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '5px' }}>
                        <input 
                            ...
                            placeholder="프리셋 이름..."
                            style={{ flex: 1, padding: '8px', backgroundColor: '#111827', border: '1px solid #4b5563', color: 'white', borderRadius: '6px', fontSize: '12px' }}
                        />
                        <button 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                  <button 
                      ...
                      disabled={loading}
                      style={{ flex: 1, padding: '15px', backgroundColor: '#e11d48', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                  >
                  ...
                  <button 
                      ...
                      style={{ flex: 1, padding: '15px', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                  >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div>
                  <textarea 
                      ...
                      style={{ width: '100%', padding: '15px', borderRadius: '8px', backgroundColor: '#111827', border: '1px solid #374151', color: '#fbbf24', resize: 'none', fontFamily: 'monospace', minHeight: '120px' }}
                  />
          ...
          {isAddPromptOpen && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, backdropFilter: 'blur(4px)' }}>
                <div style={{ backgroundColor: '#1f2937', padding: '24px', borderRadius: '16px', border: '1px solid #374151', width: '400px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                        ...
                        <input 
                            ...
                            style={{ width: '100%', padding: '10px', backgroundColor: '#374151', border: '1px solid #4b5563', color: 'white', borderRadius: '6px' }}
                        />
                    </div>
                    <div style={{ marginBottom: '24px' }}>
                        ...
                        <textarea 
                            ...
                            style={{ width: '100%', height: '100px', padding: '10px', backgroundColor: '#374151', border: '1px solid #4b5563', color: 'white', borderRadius: '6px', resize: 'none' }}
                        />
                    </div>
                ...
                <div style={{ backgroundColor: '#1f2937', padding: '24px', borderRadius: '16px', border: '1px solid #374151', width: '600px', maxWidth: '90vw', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
                    ...
                    <div style={{ color: '#d1d5db' }}>
                        <p style={{ fontSize: '13px', color: labelColor, marginBottom: '15px' }}>
                            Suno.ai에서 댄서들이 선호하는 음악을 만들기 위한 장르별 추천 설정값입니다.
                        </p>
                        ...
                        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#374151', borderRadius: '8px', fontSize: '12px', lineHeight: '1.6', color: legibilityMode ? '#FFFFFF' : 'inherit' }}>
                            ...
                            - <strong>Girl Crush:</strong> 묵직한 베이스가 중요하며, 135 BPM 근처가 군무에 적합합니다.<br/>
                            - <strong>TikTok Challenge:</strong> 15초~30초 하이라이트 구간 반복이 핵심입니다.<br/>
                            - <strong>Key:</strong> 마이너 키(Minor Key)가 K-Pop 특유의 트렌디함과 비장미를 살려줍니다.
                        </div>
                    </div>
...
const ArtTab = ({ project, onUpdate, legibilityMode }: any) => {
    ...
    return (
        <div style={{ width: '100%', height: 'calc(100vh - 150px)', display: 'grid', gridTemplateColumns: '320px 320px 1fr', gap: '20px', minHeight: '600px' }}>
                ...
                <div style={{ backgroundColor: '#1f2937', padding: '15px', borderRadius: '8px', border: '1px solid #374151' }}>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                        ...
                        <input 
                            ...
                            placeholder="Title"
                            style={{ width: '100%', padding: '10px', backgroundColor: '#111827', border: '1px solid #4b5563', color: 'white', borderRadius: '6px' }} 
                        />
                    </div>
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '5px', marginBottom: '8px' }}>
                            <input 
                                ...
                                style={{ flex: 1, padding: '10px', backgroundColor: '#111827', border: '1px solid #4b5563', color: 'white', borderRadius: '6px' }} 
                            />
                ...
                <div style={{ backgroundColor: '#1f2937', padding: '15px', borderRadius: '8px', border: '1px solid #374151' }}>
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            style={{ width: '100%', height: '60px', padding: '10px', backgroundColor: '#111827', border: '1px solid #4b5563', color: 'white', borderRadius: '6px', resize: 'none' }}
                        />
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto', paddingRight: '10px', borderRight: '1px solid #374151' }}>
                ...
                <div style={{ backgroundColor: '#1f2937', padding: '15px', borderRadius: '8px', border: '1px solid #374151' }}>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <label style={{ flex: 1, padding: '10px', borderRadius: '8px', border: modelType === 'flash' ? '1px solid #fbbf24' : '1px solid #4b5563', backgroundColor: modelType === 'flash' ? 'rgba(251, 191, 36, 0.1)' : 'transparent', cursor: 'pointer', fontSize: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: '10px', color: '#6b7280' }}>(Fast, Standard)</span>
                            </label>
                            <label style={{ flex: 1, padding: '10px', borderRadius: '8px', border: modelType === 'pro' ? '1px solid #e11d48' : '1px solid #4b5563', backgroundColor: modelType === 'pro' ? 'rgba(225, 29, 72, 0.1)' : 'transparent', cursor: 'pointer', fontSize: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: '10px', color: '#6b7280' }}>(High Quality, HD)</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                        ...
                        <div style={{ fontSize: '11px', color: labelColor, marginTop: '5px' }}>
                            {IMAGE_SIZE_PRESETS[selectedSizePreset].desc}
                        </div>
                    </div>

                    <div style={{ marginBottom: '15px' }}>
                        <label style={{ display: 'block', fontSize: '13px', color: labelColor, marginBottom: '5px' }}>생성 모드 (Mode)</label>
                        <select 
                            value={generationMode} 
                            onChange={(e) => setGenerationMode(e.target.value as any)} 
                            style={{ width: '100%', padding: '10px', backgroundColor: '#111827', color: 'white', border: '1px solid #4b5563', borderRadius: '6px', fontSize: '13px' }}
                        >
                            <option value="AI">AI Generation (Standard)</option>
                            <option value="PROMPT_ONLY">Generate Prompt Only (Text)</option>
                            <option value="MOCK">Mock Generation (Test)</option>
                        </select>
                    </div>
                    ...
                    >
                        {loading ? 'Generating...' : <><span className="material-symbols-outlined">auto_awesome</span> 앨범 커버 생성 (Generate)</>}
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
                ...
                {project.coverImage && (
                    ...
                    >
                        <span className="material-symbols-outlined">download</span> 이미지 다운로드 (Download)
                    </button>
                )}

                {generatedPrompt && (
                    <div style={{ backgroundColor: '#1f2937', padding: '15px', borderRadius: '8px', border: '1px solid #374151' }}>
                        <label style={{ display: 'block', fontSize: '12px', color: '#9ca3af', marginBottom: '5px' }}>Generated Prompt:</label>
                        <p style={{ margin: 0, fontSize: '13px', color: '#e5e7eb', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>{generatedPrompt}</p>
                    </div>
                )}
            </div>
        </div>
...
const MetadataDraftForm = ({ project, onCancel, legibilityMode }: any) => {
    ...
    const copyAll = () => {
        ...
        alert('🎵 메타데이터 초안이 복사되었습니다! (Copied to Clipboard)');
    };

    const labelColor = legibilityMode ? '#F9FAF8' : '#9ca3af';

    return (
        <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'left', backgroundColor: '#1f2937', color: 'white', borderRadius: '12px', overflow: 'hidden', border: '1px solid #374151' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
                    <div>
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <input type="text" value={project.title} readOnly style={{ width: '100%', padding: '10px', backgroundColor: '#374151', border: '1px solid #4b5563', color: 'white', borderRadius: '6px' }} />
                        </div>
                        <div style={{ marginBottom: '20px' }}>
                            <label style={{ display: 'block', color: labelColor, fontSize: '13px', marginBottom: '5px' }}>아티스트 (Artist)</label>
                            <div style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
                                <input type="text" value={artist} onChange={(e) => setArtist(e.target.value)} placeholder="Artist Name" style={{ flex: 1, padding: '10px', backgroundColor: '#374151', border: '1px solid #4b5563', color: 'white', borderRadius: '6px' }} />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <textarea value={project.lyrics || ''} readOnly style={{ flex: 1, padding: '10px', backgroundColor: '#374151', border: '1px solid #4b5563', color: '#e5e7eb', borderRadius: '6px', resize: 'none', fontFamily: 'monospace', minHeight: '300px' }} />
                    </div>
                </div>
...
const ExportTab = ({ project, onExportJSON, legibilityMode }: { project: Project, onExportJSON: () => void, legibilityMode: boolean }) => {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
          <div onClick={onExportJSON} style={{ backgroundColor: '#1f2937', padding: '50px 30px', borderRadius: '24px', border: '1px solid #374151', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', transition: 'all 0.2s', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
              ...
              <div>
                  ...
                  <p style={{ margin: 0, fontSize: '14px', color: legibilityMode ? '#E5E7EB' : '#9ca3af', lineHeight: '1.6' }}>프로젝트의 모든 설정과 데이터를 JSON 파일로 안전하게 저장합니다.</p>
              </div>
          </div>
          <div onClick={() => setShowMetadataForm(true)} style={{ backgroundColor: '#1f2937', padding: '50px 30px', borderRadius: '24px', border: '1px solid #374151', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', transition: 'all 0.2s', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
              ...
              <div>
                  ...
                  <p style={{ margin: 0, fontSize: '14px', color: legibilityMode ? '#E5E7EB' : '#9ca3af', lineHeight: '1.6' }}>Suno, YouTube 업로드를 위한 제목, 태그, 설명글을 생성합니다.</p>
              </div>
          </div>
      </div>
    </div>
...
const Studio = ({ project, onUpdate, onBack, onExportJSON, legibilityMode }: { project: Project, onUpdate: (u: Partial<Project>) => void, onBack: () => void, onExportJSON: () => void, legibilityMode: boolean }) => {
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      <nav style={{ width: '80px', backgroundColor: '#111827', borderRight: '1px solid #374151', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '20px', gap: '20px' }}>
        ...
        <div style={{ height: '20px', borderBottom: '1px solid #374151', width: '50%', margin: '10px auto' }}></div>
        <NavButton active={showManual} onClick={() => setShowManual(true)} icon="menu_book" label="매뉴얼" legibilityMode={legibilityMode} />
      </nav>
      <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#1f2937', padding: '30px' }}>
        {renderContent()}
      </div>
...
const ApiKeySelection = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#111827', color: 'white', gap: '20px' }}><div style={{ fontSize: '48px' }}>🎹</div><h1>Welcome to Suno Studio Pro</h1><p style={{ color: '#9ca3af', maxWidth: '400px', textAlign: 'center' }}>To generate high-quality content, please select a valid Google Cloud API Key.</p><button onClick={handleSelect} disabled={loading} style={{ padding: '12px 24px', backgroundColor: '#e11d48', color: 'white', borderRadius: '8px', fontWeight: 'bold' }}>{loading ? 'Connecting...' : 'Select API Key to Start'}</button></div>
  );
};

const Header = ({ view, project, onBack, onSave, onImport, onRemix, legibilityMode, onToggleLegibility, onOpenKeySettings }: any) => {
    return (
        <div style={{ height: '60px', backgroundColor: '#111827', borderBottom: '1px solid #374151', display: 'flex', alignItems: 'center', padding: '0 20px', justifyContent: 'space-between', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                {/* External API Key Manager Button */}
                <button 
                    ...
                    style={{ 
                        ...
                        backgroundColor: '#374151', 
                        color: '#fbbf24', 
                        border: '1px solid #4b5563', borderRadius: '6px', fontSize: '12px', cursor: 'pointer',
                        fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '6px'
                    }}
                    title="Manage External API Key"
                >
                ...
                {view === 'STUDIO' && (
                    <>
                        ...
                        <button onClick={onSave} style={{ padding: '8px 16px', backgroundColor: '#374151', color: '#d1d5db', borderRadius: '6px', fontSize: '13px', cursor: 'pointer', border: 'none' }}>Save File</button>
                    </>
                )}
            </div>
        </div>
...
const App = () => {
    ...
    useEffect(() => {
        const checkApiKey = async () => {
            if (getCustomKey() || process.env.API_KEY) { setApiKeySet(true); }
    ...
    return (
        <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: legibilityMode ? "'Inter', sans-serif" : "'Inter', sans-serif", backgroundColor: '#111827', color: 'white' }}>
            <Header 
                ...
                onOpenKeySettings={() => setShowKeySettings(true)}
            />
            ...
            {showKeySettings && <ApiKeySettingsModal onClose={() => setShowKeySettings(false)} />}
        </div>
    );
};
